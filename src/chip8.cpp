...
Chip8::Chip8()
    : stack{Stack<uint16_t, 16>{reg.SP}}, timer{&Chip8::timerFunc, this} {}

Chip8::~Chip8() { this->timer.join(); }
...
void Chip8::decodeInstruction(uint16_t ins) {
  ...
  case 0x8: // 8XYK
    ...
    case 0x4: { // 8XY4 Vx = Vx + Vy; Vf = 1 if result > 255, 0 otherwise
      bool carryFlag = ((static_cast<uint32_t>(this->reg.V[(ins >> 8) & 0xF]) +
  ...
  case 0xC: // CXNN Vx = randNumb & NN
    this->reg.V[(ins >> 8) & 0xF] = c8::utils::getRandomNumber() & (ins & 0xFF);
    break;
  ...
  case 0xE:                     // EX9E & EXA1
    if ((ins & 0xFF) == 0x9E) { // skip if key coresponding to Vx is pressed
      bool isKeyDown = c8::keyboard::keyDown(this->getKeyInput(),
                                             this->reg.V[(ins >> 8) & 0xF]);
      if (isKeyDown) {
        this->reg.PC += 2;
      ...
      bool isKeyUp = c8::keyboard::keyUp(this->getKeyInput(),
                                         this->reg.V[(ins >> 8) & 0xF]);
      if (isKeyUp) {
        this->reg.PC += 2;
        // this->setKeyInput(0xFF);
  ...
  case 0xF:
    ...
    case 0x0A: { // FX0A Vx = key pressed
      // uint8_t keyPressed =
      // c8::keyboard::convertScanCode(c8::keyboard::getKeyPressed());
